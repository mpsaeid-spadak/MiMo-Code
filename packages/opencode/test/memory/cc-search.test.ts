...
describe("memory.search with cc scope", () => {
  it.live("ranks CC and spadak hits in one result list", () =>
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(path.join(spadakRoot, "global", "m.md"), "distinctivetoken from spadak"),
          )
    ...
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() => fs.rm(spadakRoot, { recursive: true, force: true }))
          yield* Effect.promise(() => fs.mkdir(path.join(spadakRoot, "global"), { recursive: true }))
          yield* Effect.promise(() => fs.writeFile(path.join(spadakRoot, "global", "m.md"), "tokenz"))
