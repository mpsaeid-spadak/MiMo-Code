  ...
  itLive.live("indexes CC files under scope='cc' with frontmatter type", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        ...
        const spadakRoot = yield* memory.root()
        yield* Effect.promise(() => fs.rm(spadakRoot, { recursive: true, force: true }))
...
Body about cache.`
        ...
        yield* Effect.promise(() => reconcileMemory({ spadak: spadakRoot, cc: ccBase }))
  ...
  itLive.live("union prune: deleting one CC file leaves spadak rows untouched", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const memory = yield* Memory.Service
        const spadakRoot = yield* memory.root()
        yield* Effect.promise(() => fs.rm(spadakRoot, { recursive: true, force: true }))
        yield* Effect.promise(() => fs.mkdir(path.join(spadakRoot, "global"), { recursive: true }))
        yield* Effect.promise(() => fs.writeFile(path.join(spadakRoot, "global", "m.md"), "spadak body"))
...
cc body`,
        ...
        expect(rows.length).toBe(1)
        expect(rows[0].scope).toBe("global") // spadak row preserved
      }),
    ),
  )
...
body`,
        ...
        yield* Effect.promise(() => reconcileMemory({ spadak: spadakRoot }))
        expect(
          Database.use((db) =>
            db.select().from(MemoryFtsTable).where(eq(MemoryFtsTable.scope, "cc")).all(),
          ).length,
    ...
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        ...
        const result = yield* Effect.promise(() =>
          reconcileMemory({ spadak: spadakRoot, cc: "/definitely/not/a/real/path/abc123" }),
        )
