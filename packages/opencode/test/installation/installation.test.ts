...
describe("installation", () => {
  describe("method", () => {
    test("detects npm when /cli is in npm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "npm" && args.includes("-g")) return "/cli@1.0.0"
          return ""
        },
      )
    ...
    test("detects pnpm when /cli is in pnpm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "pnpm" && args.includes("-g")) return "/cli@1.0.0"
          return ""
        },
      )
    ...
    test("detects bun when /cli is in bun pm ls output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "bun" && args.includes("-g")) return "/cli@1.0.0"
          return ""
        },
      )
    ...
    test("returns unknown when no package manager has /cli", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        () => "",
      )
  ...
  describe("latest", () => {
    test("reads version from npm registry for npm method", async () => {
      const layer = testLayer(
        (req) => {
          expect(req.url).toContain(encodeURIComponent("/cli"))
    ...
    test("resolves version from FDS for curl method", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "curl" && args.includes("https://spadakcode.cnbj1.mi-fds.com/spadakcode/spadakcode/releases/latest"))
            return "v0.1.1\n"
          return ""
        },
      )
  ...
  describe("upgrade", () => {
    test("runs npm install with correct package", async () => {
      ...
      expect(capturedArgs).toContain("/cli@2.0.0")
    })
