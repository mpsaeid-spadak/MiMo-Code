...
// patterns like `.spadakcode/` in root .gitignore affect test results.
...
describe("file.ripgrep", () => {
    ...
    withTmpdirOutsideGit(async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          await Bun.write(path.join(dir, ".spadakcode", "thing.json"), "{}")
        },
      })
      ...
      expect(files.includes(path.join(".spadakcode", "thing.json"))).toBe(true)
    }))
  ...
  test("hidden false excludes hidden", async () => {
    ...
    expect(files.includes("visible.txt")).toBe(true)
    expect(files.includes(path.join(".spadakcode", "thing.json"))).toBe(false)
  })
