...
import { SpadakAuthPlugin } from "../../src/plugin/spadak"
...
describe("SpadakAuthPlugin", () => {
  describe("config hook", () => {
    test("registers spadak provider placeholder", async () => {
      const hooks = await SpadakAuthPlugin(fakeInput)
      const cfg: any = {}
      await hooks.config!(cfg)
      // The plugin only registers the provider so it shows up before login.
      // name/api are left to the models.dev database (name: "Spadak").
    ...
    test("does not overwrite existing config", async () => {
      const hooks = await SpadakAuthPlugin(fakeInput)
      const cfg: any = { provider: { spadak: { name: "Custom", api: "https://custom.api" } } }
      await hooks.config!(cfg)
  ...
  describe("auth hook structure", () => {
    test("registers auth for spadak provider", async () => {
      const hooks = await SpadakAuthPlugin(fakeInput)
      expect(hooks.auth).toBeDefined()
      expect(hooks.auth!.provider).toBe("spadak")
    })
  ...
  describe("authorize", () => {
    test("returns url with pk, redirect_uri, kn, key_name params", async () => {
      ...
      expect(url.searchParams.get("kn")).toBe("spadakcode")
      expect(url.searchParams.get("key_name")).toMatch(/^spadak-code-cli-key-/)
  ...
  describe("chat.headers hook", () => {
    test("adds X-Mimo-Source header for spadak provider", async () => {
      const hooks = await SpadakAuthPlugin(fakeInput)
      const output = { headers: {} as Record<string, string> }
      await hooks["chat.headers"]!({ model: { providerID: "spadak" } } as any, output as any)
      expect(output.headers["X-Mimo-Source"]).toBe("spadakcode-cli")
    })
