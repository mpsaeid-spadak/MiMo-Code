...
describe("util.env-info", () => {
  test("returns host, user, runtime, and spadakcode metadata", async () => {
    ...
    expect(info.spadakcode.version).toBeTruthy()
    expect(info.spadakcode.channel).toBeTruthy()
    expect(info.spadakcode.installation_id).toMatch(
