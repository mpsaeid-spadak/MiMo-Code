...
const root = path.resolve(import.meta.dir, "../../src/skill/builtin/.bundle/spadakcode-docs")

describe("spadakcode-docs provider guidance", () => {
  ...
  test("documents the implemented OpenAI-compatible adapter and safe merge rules", async () => {
    ...
    expect(providers).toContain("spadak models PROVIDER_ID")
  })
  ...
  test("documents the effective global config precedence", async () => {
    ...
    expect(providers).toContain("`config.json`, `spadakcode.json`, then `spadakcode.jsonc`; later files win")
    expect(providers).toContain("create `spadakcode.jsonc` when none exists")
  })
  ...
  test("keeps a newly configured API model in TUI recents without clobbering state", async () => {
    ...
    expect(providers).toContain("$SPADAKCODE_HOME/state/model.json")
    ...
    expect(providers).toContain("Write the recent state only after `spadak models PROVIDER_ID`")
...
describe("spadakcode-docs TUI troubleshooting", () => {
  test("routes rendering issues to actionable terminal and SSH guidance", async () => {
    ...
    expect(guide).toContain("spadak serve --port 4096")
    expect(guide).toContain("ssh -N -L 4096:127.0.0.1:4096 user@remote-host")
    expect(guide).toContain("spadak attach http://127.0.0.1:4096")
 ...
 */
describe("spadakcode-docs permission arity", () => {
  test("classifies mcp_sampling as glob-keyed, not action-only", async () => {
    ...
    expect(permissions).toContain('"mcp_sampling": { "*": "ask", "spadak-cut": "allow" }')
  })
