...
//
// Unlike the mock-LLM suite, this drives WorkflowRuntime against the REAL spadak
...
// The real spadak provider, read verbatim from the user's config (key included).
function realConfig() {
  const home = path.join(os.homedir(), ".config", "spadakcode", "spadakcode.json")
  const full = JSON.parse(readFileSync(home, "utf8"))
  const spadak = full.provider?.spadak
  if (!spadak?.options?.apiKey) throw new Error("no spadak provider/key in " + home)
  return { provider: { spadak } }
}

const realRef = { providerID: ProviderID.make("spadak"), modelID: ModelID.make("spadak-v2.5-pro") }
      ...
      provideTmpdirInstance(
        (dir) =>
          Effect.gen(function* () {
            ...
            // Two saved workflows on disk, resolved by name from .spadakcode/workflows.
            const wfDir = path.join(dir, ".spadakcode", "workflows")
