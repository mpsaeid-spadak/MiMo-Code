...
describe("WorkflowRuntime per-agent timeout (straggler-abort)", () => {
  // A single hung agent (e.g. a persistent spadak TTFT wall) must not stall the run
...
// the REASON to triage (spadak TTFT timeout vs spawn-reject vs over-cap). The
...
// instead of silently dropping to unbounded — which used to let a wedged spadak
