...
test("getCronFilePath joins .spadakcode/scheduled_tasks.json", () => {
  expect(getCronFilePath("/tmp/x").endsWith("/.spadakcode/scheduled_tasks.json")).toBe(true)
})
...
test("malformed cron in file is silently dropped", async () => {
  ...
  await fs.mkdir(join(dir, ".spadakcode"), { recursive: true })
  await fs.writeFile(
    getCronFilePath(dir),
    JSON.stringify({
      tasks: [
