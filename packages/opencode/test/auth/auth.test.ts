...
describe("Auth", () => {
  ...
  // in-process). It exists so credentials never have to sit in SPADAKCODE_AUTH_CONTENT, which every
  // child the engine spawns would inherit.
  it.live("inject supplies credentials without touching the environment", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        Auth.inject(JSON.stringify({ spadak: { type: "api", key: "sk-injected" } }))
        const data = yield* (yield* Auth.Service).all()
        expect(data["spadak"]).toEqual({ type: "api", key: "sk-injected" })
        expect(process.env.SPADAKCODE_AUTH_CONTENT).toBeUndefined()
  ...
  it.live("inject wins over the env channel, and clearing it falls back", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        process.env.SPADAKCODE_AUTH_CONTENT = JSON.stringify({ spadak: { type: "api", key: "sk-env" } })
        Auth.inject(JSON.stringify({ spadak: { type: "api", key: "sk-injected" } }))
        const auth = yield* Auth.Service
        expect((yield* auth.all())["spadak"]).toEqual({ type: "api", key: "sk-injected" })
        Auth.inject(undefined)
        expect((yield* auth.all())["spadak"]).toEqual({ type: "api", key: "sk-env" })
        delete process.env.SPADAKCODE_AUTH_CONTENT
      }),
    ),
  )
})
