...
const log = Log.create({ service: "plugin.spadak" })

const PLATFORM_URL = process.env.SPADAK_PLATFORM_URL || "https://platform.spadak.dev"

function getKeyName(): string {
  const filePath = path.join(Global.Path.data, "spadak-key-name")
  try {
    const existing = fs.readFileSync(filePath, "utf-8").trim()
    if (existing) return existing
  } catch {}
  const name = `spadak-code-cli-key-${crypto.randomBytes(4).toString("hex")}`
...
export async function SpadakAuthPlugin(_input: PluginInput): Promise<Hooks> {
  return {
    config: async (input) => {
      input.provider ??= {}
      // Register xiaomi as a config provider so it shows up even before login.
      // name/api are intentionally left to the models.dev database (name: "Spadak",
      // api: https://api.spadak.dev/v1) — hardcoding "Spadak" here collided with
      // the free "spadak" provider's display name and confused users.
      ...
      methods: [
        {
          ...
          authorize: async () => {
            ...
            log.info("spadak oauth server started", { port })
            ...
            const serverCallbackPromise = new Promise<{ sk?: string; uid: string; url?: string }>((resolve, reject) => {
              ...
              server.on("request", (req, res) => {
                ...
                log.info("spadak oauth callback received", { path: url.pathname, query: url.search.substring(0, 100) })

                const u = url.searchParams.get("u")

                if (!u) {
                  log.warn("spadak oauth callback missing u param")
                ...
                try {
                  ...
                  log.info("spadak oauth decrypt success", { uid: result.uid, url: result.url })
                  res.writeHead(302, { Location: `${PLATFORM_URL}/authorize/callback?status=success` })
                  res.end()
                  clearTimeout(timeout)
                  resolve(result)
                } catch (err) {
                  log.error("spadak oauth decrypt failed", { error: err })
    ...
    "chat.headers": async (input, output) => {
      if (input.model.providerID !== "spadak") return
      output.headers["X-Mimo-Source"] = "spadakcode-cli"
    },
  }
}
