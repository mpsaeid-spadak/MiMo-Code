...
export type PatchDeps = {
  ...
  files: (dir: string, name: "spadakcode" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".spadakcode")
}

function patchName(kind: Kind): "spadakcode" | "tui" {
  if (kind === "server") return "spadakcode"
  return "tui"
}
