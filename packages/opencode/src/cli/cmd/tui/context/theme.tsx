...
import spadakcode from "./theme/spadakcode.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@spadak/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light"; plain?: boolean }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = props.plain ? "system" : config.theme ?? kv.get("theme", "spadakcode")
        draft.active = typeof active === "string" ? active : "spadakcode"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.spadakcode, store.mode)
    })
...
const THEME_SCHEMA_URL = "https://spadak.dev/spadakcode/theme.json"
...
function generateSystem(colors: TerminalColors, mode: "dark" | "light"): ThemeJson {
  ...
  const spadakOrange = RGBA.fromInts(255, 103, 0)

  return {
    theme: {
      // Primary colors using Spadak Orange
      primary: spadakOrange,
      secondary: spadakOrange,
      accent: spadakOrange,

      // Status colors using ANSI
      error: ansiColors.red,
      warning: ansiColors.yellow,
      success: spadakOrange,
      info: spadakOrange,
