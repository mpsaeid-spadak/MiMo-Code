...
import { DialogSpadakLogin } from "@tui/component/dialog-spadak-login"
...
function rendererConfig(_config: TuiConfig.Info, plainTerminal: boolean): CliRendererConfig {
  const mouseEnabled = !plainTerminal && !Flag.SPADAKCODE_DISABLE_MOUSE && (_config.mouse ?? true)
...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  useKeyboard((evt) => {
    if (!Flag.SPADAKCODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.SPADAKCODE_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("SpadakCode")
      return
    }
  ...
  onMount(() => {
    if (!Flag.SPADAKCODE_EXPERIMENTAL_ORCHESTRATOR) return
  ...
  createEffect(() => {
    ...
    if (Flag.SPADAKCODE_EXPERIMENTAL_ORCHESTRATOR && !orchestratorDirResolved()) return
  ...
  createEffect(() => {
    ...
    // common case: user runs `spadak -s <id>` from a project dir) navigates the
  ...
  command.register(() => [
    ...
    },
    {
      ...
      value: "workflow.list",
      category: "session",
      enabled: Flag.SPADAKCODE_EXPERIMENTAL_WORKFLOW_TOOL,
      slash: {
        name: "workflows",
      },
      onSelect: () => {
    ...
    },
    {
      ...
      onSelect: async () => {
        await sdk.client.auth.remove({ providerID: "spadak" })
    ...
    },
    {
      ...
      onSelect: () => {
        open("https://spadak.dev/coder/docs").catch(() => {})
        dialog.clear()
      },
      category: "system",
    },
  ...
  return (
    <box
      ...
      onMouseDown={(evt) => {
        ...
        if (
          Flag.SPADAKCODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT &&
    ...
    >
      <Show when={Flag.SPADAKCODE_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
