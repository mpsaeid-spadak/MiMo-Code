...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  // Every config dir we may read from: global config dir, any `.spadakcode`
  // folders between cwd and home, and SPADAKCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.SPADAKCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit SPADAKCODE_TUI_CONFIG override, if set.
  if (Flag.SPADAKCODE_TUI_CONFIG) {
    const configFile = Flag.SPADAKCODE_TUI_CONFIG
  ...
  // 4. `.spadakcode` directories (and SPADAKCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".spadakcode") || dir === Flag.SPADAKCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".spadakcode") && dir !== Flag.SPADAKCODE_CONFIG_DIR) continue
...
const TUI_SCHEMA_URL = "https://spadak.dev/spadakcode/tui.json"
...
async function load(text: string, configFilepath: string): Promise<Info> {
  return ConfigVariable.substitute({ text, type: "path", path: configFilepath, missing: "empty" })
    ...
    .then((data) => {
      ...
      // (mirroring the old spadakcode.json shape) still get their settings applied.
