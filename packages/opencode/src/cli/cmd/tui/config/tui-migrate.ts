...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from spadakcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const spadakcode = await spadakcodeFiles(input)
  for (const file of spadakcode) {
...
async function spadakcodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "spadakcode"),
    ...(await Filesystem.findUp(["spadakcode.json", "spadakcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "spadakcode"))
  }
  if (Flag.SPADAKCODE_CONFIG) files.push(Flag.SPADAKCODE_CONFIG)
