...
export function DialogSpadakLogin() {
  ...
  const options = createMemo(() => {
    const recommended = [
      {
        title: t("tui.dialog.login.spadak"),
        value: "spadak",
        description: t("tui.dialog.login.spadak.desc"),
    ...
    return [
      ...recommended,
      ...providerOptions().filter((option) => option.value !== "spadak"),
    ]
  })
...
function MimoOAuthFlow(props: { url: string; instructions: string }) {
  ...
  async function onLoginSuccess() {
    await sdk.client.instance.dispose()
    await sync.bootstrap()
    const xiaomi = sync.data.provider.find((p) => p.id === "spadak")
    const defaultModel = xiaomi && "spadak-v2.5-pro" in xiaomi.models ? "spadak-v2.5-pro" : xiaomi ? Object.keys(xiaomi.models)[0] : undefined
    if (defaultModel) {
      local.model.set({ providerID: "spadak", modelID: defaultModel }, { recent: true })
    }
    dialog.clear()
  }
