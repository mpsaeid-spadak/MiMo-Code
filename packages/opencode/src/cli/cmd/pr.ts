...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run spadakcode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for spadakcode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found spadakcode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["spadak", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting spadakcode...")
        UI.println()

        const spadakArgs = sessionId ? ["-s", sessionId] : []
        const spadakProcess = Process.spawn(["spadak", ...spadakArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await spadakProcess.exited
        if (code !== 0) throw new Error(`spadak exited with code ${code}`)
      },
    })
  },
})
