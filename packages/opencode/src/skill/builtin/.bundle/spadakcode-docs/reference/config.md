# SpadakCode Configuration Reference

## File locations & precedence

Config is JSON or JSONC. SpadakCode discovers it by walking up from the cwd to the worktree root, then falls back to global.

- Project: `.spadakcode/spadakcode.json` or `.spadakcode/spadakcode.jsonc`
- Global: `~/.config/spadakcode/spadakcode.jsonc` or `spadakcode.json` (XDG config dir). New installs seed `spadakcode.jsonc`.
- Extra config dirs are also searched via `$SPADAKCODE_CONFIG_DIR`.

Project config merges **over** global. Always include `"$schema": "https://spadak.dev/spadakcode/config.json"` for validation.

## On-disk data layout

Base directories resolve from `SPADAKCODE_HOME` if set (must be absolute → `<home>/{data,cache,config,state}`), otherwise from XDG:

| Kind | Default location | Holds |
|------|------------------|-------|
| data | `~/.local/share/spadakcode/` | memory, logs, `builtin_skills/<version>/`, bin |
| config | `~/.config/spadakcode/` | global `spadakcode.jsonc` / `spadakcode.json` |
| cache | `~/.cache/spadakcode/` | caches, downloaded bins |
| state | `~/.local/state/spadakcode/` | runtime state, including TUI recent/favorite models in `model.json` |

Memory files live under `~/.local/share/spadakcode/memory/`:
...
- `SPADAKCODE_HOME` — override all base dirs (absolute path).
- `SPADAKCODE_CONFIG_DIR` — extra config directory to search.
- `SPADAKCODE_PURE` — run without external plugins (same as `spadak --pure`). Does **not** change models or Claude Code inheritance.
- `SPADAKCODE_DISABLE_LOG_ROTATION` — keep a single growing log file instead of rotating.
- `SPADAKCODE_TEXT_TOOL_CALL_RETRY_LIMIT` — retries when a model emits a tool call as prose markup instead of a structured call (default 2).
- `SPADAKCODE_EXPERIMENTAL_CRON` — scheduled prompts (cron/loop); **on by default**. `SPADAKCODE_DISABLE_CRON` kills it at runtime. Tune loop keepalive with `SPADAKCODE_LOOP_KEEPALIVE_BUDGET` (default 1) and `SPADAKCODE_LOOP_KEEPALIVE_DELAY_S` (default 1200).
- `SPADAKCODE_EXPERIMENTAL_TOKEN_EFFICIENCY_HEURISTIC` — shape-based compaction of bash output to save tokens; off by default.
- `SPADAKCODE_SPADAK_ONLY` — pure-Spadak mode: don't inherit Claude Code settings (CLAUDE.md, `~/.claude/CLAUDE.md`), don't read provider API keys from env, fall back to the spadak-auto model.
- **External skill roots** — default load surface is `.spadakcode` + open-standard `.agents`. Brand roots are opt-in: `SPADAKCODE_ENABLE_CLAUDE_CODE_SKILLS`, `SPADAKCODE_ENABLE_CODEX_SKILLS`, `SPADAKCODE_ENABLE_OPENCODE_SKILLS`. Turn agents off with `SPADAKCODE_DISABLE_AGENTS_SKILLS`. External scans skip dotted path segments under `skills/`.
- `SPADAKCODE_DISABLE_BUILTIN_SKILLS`, `_COMPOSE_SKILLS`, `_PROJECT_CONFIG`, `_CLAUDE_IMPORT` — feature toggles.
...
```jsonc
{
  ...
  "model_groups": {
    ...
    "standard": {
      ...
      "models": ["anthropic/claude-sonnet", "openrouter/xiaomi/spadak-v2.5"]
    },
...
- A ref without `/` is a group name. If configured, SpadakCode is **provider-aware**: it prefers a member on the caller's current provider, otherwise falls back to the group's `default`.
...
Prefer a markdown file (`.spadakcode/agent/<name>.md`, body = system prompt) for defining a custom agent/mode — see the "Custom agents & modes" section in @guide.md. Use the `agent` config key for short, inline per-agent overrides.
...
As context fills, SpadakCode auto-checkpoints (a background writer distills the conversation into `checkpoint.md`) and, near the limit, **rebuilds**: it inserts a boundary at the last successful checkpoint so earlier messages collapse to the checkpoint summary while recent messages are kept verbatim. If a checkpoint writer is still running when a rebuild is needed, the rebuild waits for it (with a visible "Preparing conversation context…" status) — briefly when a usable checkpoint already exists, longer for the very first one — then proceeds; if no checkpoint can be produced it falls back to lossy compaction. You can trigger a rebuild yourself any time with the `/rebuild` slash command.

The trigger is the model's prompt capacity (`limit.input` when the provider publishes one, else `limit.context`) minus the reserves, optionally lowered by `compaction.max_context`. `spadak models <provider>` prints the resolved window and the trigger per model, and `/status` shows both plus the current usage. Two things users hit here: a model's usable window depends on the route (ChatGPT/Codex subscription vs direct API key vs a reseller like OpenRouter can all differ for the same model name, and a 1M catalog figure does not mean the route serves 1M), and providers may price very long prompts higher (OpenAI charges 2x input / 1.5x output for GPT-5.6 requests above 272K input) — `compaction.max_context` is the knob for both.
...
| `voice.asr_model` | ASR model (default `xiaomi/spadak-v2.5-asr`) |
| `voice.control_model` | Voice control model (default `xiaomi/spadak-v2.5`) |
...
| `server` | Config for `spadak serve` |
