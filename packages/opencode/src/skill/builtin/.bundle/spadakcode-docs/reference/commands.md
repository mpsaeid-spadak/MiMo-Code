# SpadakCode Commands Reference

## CLI (`spadak <command>`)

Invoked from the shell. `spadak` with no command opens the TUI.

| Command | Purpose |
|---------|---------|
| `spadak` | Launch the interactive TUI |
| `spadak run` | Headless, non-interactive run (scripting/eval) |
| `spadak mcp` | Manage / inspect MCP servers |
| `spadak agent` | Manage agents |
| `spadak models` | List available models |
| `spadak providers` | List / manage providers |
| `spadak account` (console) | Account / login console |
| `spadak upgrade` | Update to the latest version |
| `spadak uninstall` | Uninstall SpadakCode |
| `spadak serve` | Run the server |
| `spadak llm-server issue`/`list`/`revoke` | Mint and manage tokens that let a task reach this instance's models over `/v1`; it starts nothing — see @capability-api.md |
| `spadak stats` | Usage statistics |
| `spadak export` / `spadak import` | Export / import sessions |
| `spadak session` | Manage sessions |
| `spadak github` / `spadak pr` | GitHub / pull-request integration |
| `spadak generate` | Code generation entry |
| `spadak plugin` (plug) | Manage plugins |
| `spadak db` | Database utilities |
| `spadak acp` / `spadak attach` | ACP / attach to a running session |
| `spadak debug` | Debug utilities |
| `spadak completion` | Generate shell completion script |

Run `spadak <command> --help` for flags on any command.
...
For terminal compatibility, TUI rendering or lag, and local rendering over SSH with `spadak serve` + `spadak attach`, see @guide.md.

## Slash commands (inside the TUI)

Type `/` to see the commands available in the current context. You can also ask in chat, for example, “Which slash commands can I use?” or “How do I switch models?” SpadakCode will explain the relevant command without requiring you to remember its name.
...
| `/login` | — | Sign in to Spadak |
| `/connect` | — | Connect or sign in to a model provider |
| `/logout` | — | Sign out of Spadak |
...
| `/voice` | Toggle streaming voice input (requires `sox` and a Spadak login) |
...
- Voice ASR (`spadak-v2.5-asr`) is Spadak-platform only; voice control (`spadak-v2.5`) also runs on OpenRouter and compatible relays via the `voice` config (see config.md and the README voice section).
