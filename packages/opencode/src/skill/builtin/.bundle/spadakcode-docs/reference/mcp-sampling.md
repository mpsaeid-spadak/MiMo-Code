...
SpadakCode implements the MCP client `sampling` capability
([spec](https://modelcontextprotocol.io/specification/2025-11-25/client/sampling)).
An MCP server can ask SpadakCode to run a model call on its behalf via
...
The Spadak Cut MCP server extracts a video's audio track to a 16 kHz mono WAV and
needs a transcript. Instead of shipping its own provider credentials, it sends
the WAV to SpadakCode as MCP `AudioContent` and asks for a completion:
...
```json
{
  ...
  "params": {
    ...
    "maxTokens": 2048,
    "modelPreferences": { "hints": [{ "name": "spadak-v2.5" }] }
  }
}
```

SpadakCode picks a configured, audio-capable model (preferring the hinted
`spadak-v2.5`), asks the user to approve, runs the call, and returns the model's
text unchanged. `SPADAK_API_KEY` is never read or needed by the server.

## Capability negotiation

SpadakCode declares `capabilities.sampling = {}` during `initialize`.
...
SpadakCode does exactly that. `includeContext` defaults to `none`: SpadakCode never
...
When nothing is eligible, SpadakCode returns a **structured error** naming every
...
SpadakCode is. It fails with `sampling stalled: the model produced no output` and
...
number.** Set `chunkTimeout` for a provider in `spadakcode.json` and it governs
...
Reaching this bound aborts the provider call as well as SpadakCode's wait
...
While the model call is in flight SpadakCode emits periodic
...
SDK's 60 s default gives up at 60 s while SpadakCode is still working, since
SpadakCode imposes no deadline of its own on the model call.
...
- **It reports observed output, but it is still not a fraction.** The model call
  ...
  actually produced — which is what lets a server tell "SpadakCode is alive" from
...
- **It never carries the model's text.** Partial content is deliberately withheld:
  ...
  channel, and treating `message` as one would be SpadakCode deciding that on the
...
- **It is necessary, not sufficient.** Resetting the timer is the *requester's*
  ...
  its own deadline. SpadakCode cannot make that decision for it.
- **It cannot extend SpadakCode's own bounds.** The notifications reset the peer's
...
SpadakCode reads it back off the request's `_meta`. No token means no notifications
at all.
...
A sampling request normally arrives **while SpadakCode is still waiting for that
...
2. SpadakCode runs the work on a fresh root Effect fiber, which shares no fiber,
...
>
> SpadakCode does not work around this, because it cannot: the id at risk belongs to
...
> it issues on a connection, SpadakCode does not learn of it, so that request keeps a
> model call running — and a paid one — until one of SpadakCode's own bounds aborts
...
- The model's text is returned **verbatim**. SpadakCode does not summarise or
  rewrite it.
...
that is now the only deadline SpadakCode imposes; the `"approval"`, `"model"` and
