# SpadakCode Permissions Reference
...
`mcp_sampling` gates MCP **client-side sampling**: an MCP server asking SpadakCode to run a model call on its behalf (`sampling/createMessage`), so the server needs no API key of its own. The glob argument is the MCP server name, so you can allow one server and keep the rest at `ask`:

```jsonc
{
  "permission": {
    "mcp_sampling": { "*": "ask", "spadak-cut": "allow" }
  }
}
```
...
| Surface | How |
|---------|-----|
| TUI (`spadak`) | `spadak --dangerously-skip-permissions` or `spadak --yolo` |
| TUI at runtime | `/skip-permissions` — toggle mid-session, instance-wide, inherited by subagents |
| Headless (`spadak run`) | `spadak run --dangerously-skip-permissions "<prompt>"` or `spadak run --yolo "<prompt>"` |
| Any surface (env) | `SPADAKCODE_PERMISSION='"allow"'` or `SPADAKCODE_DANGEROUSLY_SKIP_PERMISSIONS=1` |

The `/skip-permissions` toggle auto-allows permission asks but keeps `deny` rules in force; forced-ask operations (e.g. destructive bash) still prompt and auto-reject after 60s (`SPADAKCODE_SKIP_ALL_FORCED_ASK_TIMEOUT_MS`) with feedback the model can act on, so unattended runs don't hang.
...
- A top-level catch-all like `"permission": { "*": "ask" }` makes the TUI/env form a no-op (your `*: ask` outranks the injected `*: allow`). Remove it, or use `spadak run --dangerously-skip-permissions`, which auto-replies at the event layer and overrides `ask` too.
...
- `external_directory` governs reads/writes outside the project working directory — by default these prompt, so SpadakCode never silently widens scope.
