...
Every SpadakCode server already serves an OpenAI-compatible surface over the models this
project has configured, mounted at `/v1`. There is no separate LLM server to start and no
daemon to manage: if a SpadakCode process is listening, the endpoints are there.
...
behaves as it always did, credential-free on loopback — it is what `spadak attach` over SSH
uses:

```bash
spadak --port 4096                 # TUI plus a listener on a port you chose
spadak serve --port 4096           # headless, same endpoints
```
...
`spadak llm-server issue` can print a base URL. A spawned child should be **told** its
endpoint by whoever spawned it rather than picking one out of that directory.

## Minting a token

`spadak llm-server` only mints and manages credentials; it starts nothing.

```bash
spadak llm-server issue --json --label my-skill
spadak llm-server issue --model anthropic/claude-haiku-4-5 --ttl 12h
spadak llm-server list [--json]
spadak llm-server revoke <id> | --all
```
...
- `--json` prints `api_key`, `id`, `base_url`, `expires_at`, `models`, plus `renew_argv` /
  ...
  expiry without knowing how SpadakCode was invoked. `base_url` is `null` when nothing is
...
| 502 | — | The upstream provider failed, not SpadakCode |
