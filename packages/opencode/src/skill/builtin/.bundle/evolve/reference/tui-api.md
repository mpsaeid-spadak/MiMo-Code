...
```tsx
import type { TuiPlugin, TuiPluginModule } from "/plugin/tui"
...
- `~/.config/spadakcode/tui/*.{ts,tsx,js}` — global
- `<project>/.spadakcode/tui/*.{ts,tsx,js}` — project-local

Plugins can also be declared in `tui.json` (global `~/.config/spadakcode/tui.json`
or project `.spadakcode/tui.json`) via the `plugin` array (file URLs or npm specs).
...
```ts
// The full /sdk client — same as what plugins get on server side
