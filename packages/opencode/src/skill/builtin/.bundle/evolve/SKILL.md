...
Before creating: check whether the extension already exists (`ls .spadakcode/tools .spadakcode/hooks .spadakcode/skills`). Prefer improving an existing one over adding a near-duplicate.
...
Need to redo a multi-agent run  → workflow (.spadakcode/workflows/*.js)
Need to change the UI           → TUI plugin (.spadakcode/tui/*.tsx)
```
...
Write to `.spadakcode/tools/<name>.ts`:
...
Write to `.spadakcode/hooks/<name>.ts` — export a Hooks object:
...
Write to `.spadakcode/skills/<name>/SKILL.md`:
...
|------|------|-----------|
| Tools | `.spadakcode/tools/*.ts` | next turn |
| Hooks | `.spadakcode/hooks/*.ts` | next turn |
| Skills | `.spadakcode/skills/*/SKILL.md` | next turn |
| Workflows | `.spadakcode/workflows/*.js` | on invoke |
| TUI | `.spadakcode/tui/*.tsx` | restart |
