...
> **Bundled runtime:** when the `SPADAK_PYTHON` environment variable is set, skip the installs below — run every command with `python3`/`uv run` replaced by `"$SPADAK_PYTHON"` (pypdf/pypdfium2/reportlab/Pillow preinstalled; pip console scripts unavailable, use `"$SPADAK_PYTHON" -m <module>`). A bundled qpdf is exposed as `SPADAK_QPDF` (picked up automatically by the scripts here); invoke it directly as `"$SPADAK_QPDF" --check file.pdf`.
