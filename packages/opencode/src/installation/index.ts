...
export const USER_AGENT = `spadakcode/${InstallationChannel}/${InstallationVersion}/${Flag.SPADAKCODE_CLIENT}`
...
// TODO(spadakcode): uncomment when corresponding channels are supported
...
export class Service extends Context.Service<Service, Interface>()("@spadakcode/Installation") {}
...
export const layer: Layer.Layer<Service, never, HttpClient.HttpClient | ChildProcessSpawner.ChildProcessSpawner> =
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      // TODO(spadakcode): uncomment when spadakcode is published to homebrew
      ...
      const upgradeCurl = Effect.fnUntraced(
        function* (target: string) {
          ...
          const response = yield* httpOk.execute(HttpClientRequest.get(process.env.SPADAKCODE_INSTALL_SCRIPT_URL ?? "https://spadak.dev/install"))
      ...
      const upgradeCurlWindows = Effect.fnUntraced(function* (target: string) {
        ...
        const stageDir = path.join(os.tmpdir(), `spadakcode_upgrade_${pid}`)

        // Download new version to staging dir (reuses install.ps1 logic)
        const installScriptUrl = process.env.SPADAKCODE_INSTALL_SCRIPT_URL ?? "https://spadak.dev/install.ps1"
        const downloadResult = yield* run(
          ...
          { env: { SPADAKCODE_INSTALL_DIR: stageDir, VERSION: target, INSTALL_SCRIPT_URL: installScriptUrl } },
        )
        if (downloadResult.code !== 0) return downloadResult

        // Replace in-place: Windows allows renaming a running exe
        const stagedExe = path.join(stageDir, "spadak.exe")
      ...
      const methodImpl = Effect.fn("Installation.method")(function* () {
        if (process.execPath.includes(path.join(".spadakcode", "bin"))) return "curl" as Method
        ...
        const checks: Array<{ name: Method; command: () => Effect.Effect<string> }> = [
          ...
          // TODO(spadakcode): uncomment when spadakcode is published to these channels
      ...
      const latestImpl = Effect.fn("Installation.latest")(function* (installMethod?: Method) {
        ...
        if (detectedMethod === "curl") {
          ...
          // SPADAK_FDS_BASE to mirror the install script.
          const base = (process.env.SPADAK_FDS_BASE || "https://spadakcode.cnbj1.mi-fds.com/spadakcode/spadakcode").replace(
            /\/+$/,
            "",
          )
        ...
        // TODO(spadakcode): uncomment when spadakcode is published to chocolatey
        ...
        // TODO(spadakcode): uncomment when spadakcode is published to scoop
        ...
        // TODO(spadakcode): uncomment when spadakcode has github releases
        // const response = yield* httpOk.execute(
      ...
      const upgradeImpl = Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        ...
        if (!result || result.code !== 0) {
          // TODO(spadakcode): restore choco-specific error when choco channel is supported
