...
export const create = fn(CreateInput, async (input) => {
  ...
  const env = {
    SPADAKCODE_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
    SPADAKCODE_WORKSPACE_ID: config.id,
    SPADAKCODE_EXPERIMENTAL_WORKSPACES: "true",
...
async function startSync(space: Info) {
  if (!Flag.SPADAKCODE_EXPERIMENTAL_WORKSPACES) return
