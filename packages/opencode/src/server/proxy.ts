...
function headers(req: Request, extra?: HeadersInit) {
  ...
  out.delete("x-spadakcode-directory")
  out.delete("x-spadakcode-workspace")
  if (!extra) return out
...
const app = (upgrade: UpgradeWebSocket) =>
  new Hono().get(
    "/__workspace_ws",
    upgrade((c) => {
      const url = c.req.header("x-spadakcode-proxy-url")
      const queue: Msg[] = []
...
export function websocket(
  ...
  next.set("x-spadakcode-proxy-url", socket(target))
  for (const [key, value] of new Headers(extra).entries()) {
    next.set(key, value)
  }
