...
import { ensureSpadakcodeGitignore } from "@/config/gitignore"
...
export const getLockFilePath = (dir?: string) => join(dir ?? process.cwd(), ".spadakcode", ".cron-lock")
...
export const tryAcquireSchedulerLock = (opts?: { dir?: string; lockIdentity?: string }) =>
  Effect.gen(function* () {
    ...
    yield* Effect.tryPromise({
      try: () => ensureSpadakcodeGitignore(join(path, "..")),
      catch: () => undefined,
    }).pipe(Effect.orElseSucceed(() => undefined))
