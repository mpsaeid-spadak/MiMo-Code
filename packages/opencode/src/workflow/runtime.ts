...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const launch = Effect.fn("WorkflowRuntime.launch")(function* (input: StartInput, runID: string, name: string) {
      ...
      // TEST SEAM: SPADAKCODE_TEST_SPAWN_FAIL_ONCE=<n> makes the next <n> shared
      ...
      let testSpawnFailsLeft = Number(process.env.SPADAKCODE_TEST_SPAWN_FAIL_ONCE ?? 0) || 0
        ...
        yield* launch(
          {
            ...
            // let a wedged spadak TTFT stall the resumed run forever.
