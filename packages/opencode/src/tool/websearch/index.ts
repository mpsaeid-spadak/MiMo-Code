...
import * as SpadakWebsearch from "./spadak"
import { Auth } from "@/auth"
import { Provider } from "@/provider"
import DESCRIPTION from "./websearch.txt"

const WEBFETCH_FALLBACK =
  "Web search unavailable. Use `webfetch` with a relevant URL instead, or enable the Web Search plugin at https://platform.spadak.dev/console/plugin."
    ...
    return {
      ...
      execute: (params: z.infer<typeof Parameters>, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          const result =
            model?.providerID === "spadak"
              ? yield* Effect.catchCause(
                  Effect.gen(function* () {
                    const info = yield* auth.get("spadak")
                    if (!info || info.type !== "api") return undefined
                    return yield* SpadakWebsearch.call(
                      http,
                      model.api.url,
                      info.key,
                      params.query,
