...
async function setupProjectIdEnvironment(workingDir: string): Promise<void> {
  ...
  const localFile = nodePath.join(workingDir, ".spadakcode-project-id")
  const idFile = nodePath.join(mainGit, "spadakcode-project-id")
  ...
  // Belt-and-suspenders: ensure .git/info/exclude lists .spadakcode-project-id
  ...
  if (!existing.includes(".spadakcode-project-id")) {
    await nodeFs.appendFile(excludeFile, "\n.spadakcode-project-id\n")
  }
}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const fakeVcs = Schema.decodeUnknownSync(Schema.optional(ProjectVcs))(Flag.SPADAKCODE_FAKE_VCS)
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      const data: DiscoveryResult = yield* Effect.gen(function* () {
        if (Flag.SPADAKCODE_DISABLE_GIT) {
          return {
            id: ProjectID.global,
            worktree: directory,
            sandbox: directory,
        ...
        // turn, which has shipped as a bug for users opening spadakcode in $HOME.
      ...
      if (Flag.SPADAKCODE_EXPERIMENTAL_ICON_DISCOVERY) yield* discover(existing).pipe(Effect.ignore, Effect.forkIn(scope))
