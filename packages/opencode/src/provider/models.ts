...
const catalog = lazy(() => {
  const source = Flag.SPADAKCODE_MODELS_URL || "https://models.dev"
  ...
  return createCatalog({
    cache,
    explicit: Flag.SPADAKCODE_MODELS_PATH,
    snapshot: async () => {
    ...
    disabled: () => Flag.SPADAKCODE_DISABLE_MODELS_FETCH || process.argv.includes("--get-yargs-completions"),
...
/**
 * `force` always fetches (even under `SPADAKCODE_DISABLE_MODELS_FETCH`).
