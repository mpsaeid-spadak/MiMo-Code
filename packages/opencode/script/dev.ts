...
// checkout (at ../../spadakapi/packages/opencode/src/ext), it is copied into
...
const overlaySrc = path.resolve(pkgDir, "../../spadakapi/packages/opencode/src/ext")
...
const proc = Bun.spawn(["bun", "run", "--conditions=browser", "src/index.ts", ...process.argv.slice(2)], {
  ...
  env: { ...process.env, SPADAKCODE_HOME: process.env.SPADAKCODE_HOME ?? path.resolve(pkgDir, "../../.dev-home") },
})
