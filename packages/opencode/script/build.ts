...
const BINARY_PREFIX = "spadakcode"
...
if (!fs.existsSync(extDir)) {
  const overlaySrc = path.resolve(dir, "../../spadakapi/packages/opencode/src/ext")
...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    compile: {
      ...
      outfile: `dist/${name}/bin/spadak`,
      execArgv: [`--user-agent=spadakcode/${Script.version}`, "--use-system-ca", "--"],
      windows: {},
    },
  ...
  if (item.os === process.platform && item.arch === process.arch && !item.abi) {
    const binaryPath = `dist/${name}/bin/spadak`
  ...
  await Bun.file(`dist/${name}/README.md`).write(
    `This is the ${item.os}-${item.arch} binary for [/cli](https://www.npmjs.com/package//cli). Install that package directly.\n`,
  )
    ...
    JSON.stringify(
      {
        ...
        description: "Platform-specific binary for /cli.",
        license: "MIT",
        author: "Spadak Spadak Team",
        homepage: "https://spadak.dev/coder",
        repository: {
          type: "git",
          url: "git+https://github.com/SpadakSpadak/Spadak-Code.git",
        },
        keywords: ["ai", "coding", "agent", "cli", "spadak"],
        os: [item.os],
        cpu: [item.arch],
      },
      null,
...
if (Script.release) {
  ...
  // Also publish to Spadak FDS (fast download in mainland China; the install
  // script reads from there). Skipped when credentials are absent so local
  // release builds still work.
  if (process.env.SPADAK_FDS_AK && process.env.SPADAK_FDS_SK) {
    ...
    console.log("Skipping FDS upload (SPADAK_FDS_AK / SPADAK_FDS_SK not set)")
  }
}
