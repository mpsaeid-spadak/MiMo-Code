<#
.SYNOPSIS
    SpadakCode installer for Windows.
.DESCRIPTION
    Downloads and installs SpadakCode to $env:USERPROFILE\.spadakcode\bin,
...
.LINK
    https://spadak.dev/coder
.EXAMPLE
    irm https://spadak.dev/install.ps1 | iex
.EXAMPLE
    $env:VERSION = "0.1.0"; irm https://spadak.dev/install.ps1 | iex
#>
param(
    [String] $Version,
    [Switch] $NoModifyPath
...
$Arch = if ($env:PROCESSOR_ARCHITECTURE -eq "ARM64") { "arm64" }
    ...
    else { Write-Err "SpadakCode requires a 64-bit operating system." }
...
$FdsBase = if ($env:SPADAK_FDS_BASE) { $env:SPADAK_FDS_BASE.TrimEnd('/') } else {
    "https://spadakcode.cnbj1.mi-fds.com/spadakcode/spadakcode"
}
...
# Warn about old versions that don't support spadak upgrade via install.ps1
...
if ($MajMin -match '^\d+\.\d+\.\d+$') {
    ...
    if ($semver -le 104) {
        Write-Host ""
        Write-Host "WARNING: Installing v$Version via install.ps1 will cause 'spadak upgrade' to not function properly." -ForegroundColor Yellow
...
$Staging = $false
if ($env:SPADAKCODE_INSTALL_DIR) {
    $InstallDir = $env:SPADAKCODE_INSTALL_DIR
    $Staging = $true
} else {
    $InstallDir = Join-Path $env:USERPROFILE ".spadakcode\bin"
}

if (-not $Staging) {
    $Existing = Get-Command spadak -ErrorAction SilentlyContinue
    if ($Existing) {
        $InstalledVersion = & spadak --version 2>$null
        if ($InstalledVersion -eq $Version) {
            Write-Host "SpadakCode v$Version is already installed." -ForegroundColor DarkGray
            Exit-Install 0
        }
...
$BinName = if (Test-Path (Join-Path $TmpDir "spadak.exe")) { "spadak.exe" } else { "spadak" }
try {
    Move-Item -Path (Join-Path $TmpDir $BinName) -Destination (Join-Path $InstallDir "spadak.exe") -Force -ErrorAction Stop
} catch {
    Remove-Item -Recurse -Force $TmpDir -ErrorAction SilentlyContinue
    Write-Err "Failed to install binary. If SpadakCode is currently running, please close it and retry.`n$($_.Exception.Message)"
}
Remove-Item -Recurse -Force $TmpDir -ErrorAction SilentlyContinue
...
Write-Host "https://spadak.dev/coder/docs"
Write-Host ""
