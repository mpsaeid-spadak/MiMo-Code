...
**What was built** — External skill discovery defaults to spadakcode +
open-standard `.agents`. Brand roots (`.claude` / `.codex` / `.opencode`) are
opt-in via `SPADAKCODE_ENABLE_*_SKILLS`; agents turns off with
`SPADAKCODE_DISABLE_AGENTS_SKILLS`. External scans no longer match dotted path
segments, so host-private namespaces such as Codex `skills/.system` never
enter the catalog. `SPADAKCODE_DISABLE_EXTERNAL_SKILLS` and brand
`SPADAKCODE_DISABLE_*_SKILLS` skill gates are gone; `SPADAK_ONLY` /
`SPADAKCODE_DISABLE_CLAUDE_CODE` no longer affect skill roots. Same-name clashes
are layered last-wins (home brands → project brands → spadakcode → paths/urls),
...
plugin/skill installers). SpadakCode scans `~/.codex` with
...
2. **All brand roots default-on** — `.claude` / `.codex` / `.opencode` are
   ...
   `*_DISABLE_CLAUDE_CODE*` / `SPADAKCODE_SPADAK_ONLY` gates are a blunt Claude
...
roots). Native patterns (`SPADAKCODE_SKILL_PATTERN`, `SKILL_PATTERN`,
...
### S2.2 Default root set = spadakcode + agents; brand roots are opt-in

| Root | Default | Control |
| --- | --- | --- |
| `.spadakcode/skill(s)`, config `skills.paths` / `skills.urls`, builtin & compose bundles | on | existing dedicated flags only |
| `.agents/skills` (home + project up) | **on** | `SPADAKCODE_DISABLE_AGENTS_SKILLS` |
| `.claude/skills` | **off** | `SPADAKCODE_ENABLE_CLAUDE_CODE_SKILLS` |
| `.codex/skills` | **off** | `SPADAKCODE_ENABLE_CODEX_SKILLS` |
| `.opencode/skills` | **off** | `SPADAKCODE_ENABLE_OPENCODE_SKILLS` |
...
spadakcode-docs describe live behavior only — they do not list retired key names.

| Env | Default | Meaning |
| --- | --- | --- |
| `SPADAKCODE_DISABLE_AGENTS_SKILLS` | unset = agents on | Turn off the open-standard `.agents` root |
| `SPADAKCODE_ENABLE_CLAUDE_CODE_SKILLS` | unset = off | Opt in `.claude/skills` |
| `SPADAKCODE_ENABLE_CODEX_SKILLS` | unset = off | Opt in `.codex/skills` |
| `SPADAKCODE_ENABLE_OPENCODE_SKILLS` | unset = off | Opt in `.opencode/skills` |

`SPADAKCODE_DISABLE_EXTERNAL_SKILLS` and brand `SPADAKCODE_DISABLE_*_SKILLS` keys
are no longer part of the skill-root surface (changelog / PR description only).
`SPADAKCODE_SPADAK_ONLY` / `SPADAKCODE_DISABLE_CLAUDE_CODE` still govern Claude
...
Remove the outer `if (!Flag.SPADAKCODE_DISABLE_EXTERNAL_SKILLS)` guard around
external discovery. `SPADAKCODE_SKILL_PATTERN` keeps accepting `skill` and
`skills` under `.spadakcode/` for compatibility; shipped prompts may mention
...
4. `.spadakcode` / config skill dirs
...
- A **project** brand skill can still beat a **home** `.agents` skill (more
  ...
  scopes.
- `.spadakcode` and explicit `skills.paths`/`urls` beat every external root.

### S2.4 Desktop alignment (`spadak-desktop`)
...
if (!compat.agents) env.SPADAKCODE_DISABLE_AGENTS_SKILLS = "true"
if (compat.claude) env.SPADAKCODE_ENABLE_CLAUDE_CODE_SKILLS = "true"
if (compat.codex) env.SPADAKCODE_ENABLE_CODEX_SKILLS = "true"
if (compat.opencode) env.SPADAKCODE_ENABLE_OPENCODE_SKILLS = "true"
return env
```

| Desktop prefs | Injected env | Engine scan set |
| --- | --- | --- |
| default | `{}` | spadakcode + agents |
| all off | `{ SPADAKCODE_DISABLE_AGENTS_SKILLS: "true" }` | spadakcode/config/bundle only |
| agents + codex | `{ SPADAKCODE_ENABLE_CODEX_SKILLS: "true" }` | + `.codex` user skills (no `.system`) |
...
Write authority stays the spadakcode skills root; brand roots remain read-only
compatibility (existing skill-path-compat contract).

### S2.5 Docs

- `README*.md` env tables and `spadakcode-docs` `reference/config.md`: **Active
...
- Deleting `SPADAKCODE_SPADAK_ONLY` / `SPADAKCODE_DISABLE_CLAUDE_CODE*` from
...
- [x] T1: External skill scans use non-dot glob matching — acceptance: with a
  ...
  `SPADAKCODE_ENABLE_CODEX_SKILLS=true`, discovery lists only `user-skill`;
...
- [x] T2: Flip engine external-root defaults and replace env gates —
  ...
  `.claude`/`.codex`/`.opencode`; each `SPADAKCODE_ENABLE_*_SKILLS` opts in only
  its root; `SPADAKCODE_DISABLE_AGENTS_SKILLS` drops agents; setting
  `SPADAKCODE_DISABLE_EXTERNAL_SKILLS` or brand `SPADAKCODE_DISABLE_*_SKILLS`
  does not change the predicate; `SPADAKCODE_SPADAK_ONLY` /
  `SPADAKCODE_DISABLE_CLAUDE_CODE` do not affect skill roots. (covers: S2.2;
  S2.3; depends: T1)
...
- [x] T4: Align Desktop `engineSkillScanEnvFromCompat` and related unit tests —
  ...
  `SPADAKCODE_ENABLE_*_SKILLS`; all-off injects only
  `SPADAKCODE_DISABLE_AGENTS_SKILLS`; injected keys are exactly the four Active
  keys; unit matrix matches S2.4. (covers: S2.4; depends: T2)
- [x] T5: Document Active env surface — acceptance: README and spadakcode-docs
  list only the four Active keys as controls, state default load surface =
  spadakcode + agents with brand roots opt-in, and do not list retired key names
