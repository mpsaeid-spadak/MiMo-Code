---
name: drive-spadak
description: Use when developing or testing the SpadakCode repository and you need to programmatically drive another SpadakCode (spadak) process. Supports headless `spadak run` with JSON events and interactive TUI via tmux for behavior, integration, and visual regression testing. Covers an installed `spadak` binary or a dev build launched from source with `bun dev`; do not use it for ordinary SpadakCode task execution.
---
...
Two **interfaces** for driving a separate spadak process:

| Interface | Command | Use when |
|------|---------|----------|
| **Headless** | `spadak run --format json` | Scripted tasks, CI, event validation |
| **TUI** | `spadak` in tmux | Interactive flow, permission dialogs, keybindings, visual regression |

**Core principle:** Every run produces parseable evidence. No eyeballing.

Always drive an *isolated* instance: give each run a fresh `SPADAKCODE_HOME` and a throwaway workspace so it never touches your own config, memory, or session DB.
...
|----------|---------|----------|
| **Installed binary** | `spadak …` | Testing a released/installed build |
| **Dev (from source)** | `bun dev …` | Debugging spadakcode itself — runs `src/index.ts` directly, no build step, picks up local code changes |

Everything below uses `spadak` for brevity. To drive a **dev build** instead,
substitute `bun dev` for `spadak` and run from the repo root — every flag,
JSON event, and tmux technique is identical. See the Dev Mode section.

## Prerequisites

```bash
# spadak binary must be on PATH (installed-binary launcher)
which spadak || echo "spadak not found on PATH"

# OR: dev launcher — run from the spadakcode repo root, needs bun
...
## Dev Mode (debugging spadakcode itself)

When the goal is to debug **spadakcode's own code**, launch from source with
`bun dev` instead of the installed `spadak` binary. It runs
...
- It is the dev equivalent of the `spadak` command: same CLI, same subcommands
...
- Args pass straight through, so **both interfaces work under dev**:
  ...
  - TUI:      `bun dev <workspace>` (positional workspace arg, as with `spadak`)
- If `SPADAKCODE_HOME` is not set, dev defaults it to a repo-local `.dev-home`
  dir. For an isolated driven run, set `SPADAKCODE_HOME=$(mktemp -d)` explicitly
  just like with the binary.

**Substitution rule:** anywhere Part 1 / Part 2 / Part 3 below say `spadak`,
replace it with `bun dev` (invoked from the repo root) to drive a dev build.

```bash
# Headless, dev build (from repo root)
REPO=/path/to/spadakcode/checkout   # your local spadakcode repo root
SPADAKCODE_HOME=$(mktemp -d) bun --cwd "$REPO" dev run \
  --format json --dangerously-skip-permissions --dir "$WORKSPACE" \
  < "$PROMPT" > /tmp/spadak-dev.jsonl 2>&1

# TUI, dev build in tmux (from repo root)
tmux new-session -d -s "$SESSION" -x 120 -y 30 \
  "cd $REPO && SPADAKCODE_HOME=$(mktemp -d) SPADAKCODE_PURE=true bun dev $WORKSPACE; sleep 999"
```

---

## Part 1: Headless Mode (`spadak run`)

### Launch

```bash
PROMPT=$(mktemp -t spadak-drive.XXXXXX)
cat >"$PROMPT" <<'EOF'
Your task here.
EOF

SPADAKCODE_HOME=$(mktemp -d) spadak run \
  --format json \
  --dangerously-skip-permissions \
  --dir "$WORKSPACE" \
  < "$PROMPT" > /tmp/spadak-out.jsonl 2>&1
...
# No errors
grep -q '"type":"error"' /tmp/spadak-out.jsonl && echo "FAIL: errors found"

# A specific tool was used (match part.tool, the bare string)
grep -q '"tool":"write"' /tmp/spadak-out.jsonl || echo "FAIL: write tool not called"

# Text output contains expected string (text is at .part.text)
grep '"type":"text"' /tmp/spadak-out.jsonl | jq -r '.part.text' | grep -q "expected"

# Robust tool check via jq (works regardless of key ordering)
jq -e 'select(.type=="tool_use") | .part.tool=="write"' /tmp/spadak-out.jsonl >/dev/null \
...
```bash
timeout 120 spadak run --format json --dangerously-skip-permissions < "$PROMPT"
...
| Variable | Purpose |
|---|---|
| `SPADAKCODE_HOME` | **Required.** Fresh `mktemp -d` per run. Sandboxes DB, config, cache. |
| `SPADAKCODE_PURE=true` | Disable external plugins. |
| `SPADAKCODE_DISABLE_GIT=true` | Skip git ops if workspace isn't a real repo. |
...
MHOME=$(mktemp -d)
WORKSPACE=$(mktemp -d)
SESSION="spadak-drive-$$"

# Launch spadak TUI in tmux (workspace is a positional arg, NOT --dir)
tmux new-session -d -s "$SESSION" -x 120 -y 30 \
  "SPADAKCODE_HOME=$MHOME SPADAKCODE_PURE=true spadak $WORKSPACE; sleep 999"
...
test_smoke() {
  ...
  echo "Say hello" > "$P"
  SPADAKCODE_HOME=$MHOME spadak run --format json --dangerously-skip-permissions \
    < "$P" > /tmp/s1.jsonl 2>&1
  local E=$?
  rm -rf "$MHOME" "$P"
...
test_tool_use() {
  ...
  SPADAKCODE_HOME=$MHOME spadak run --format json --dangerously-skip-permissions --dir "$WS" \
    < "$P" > /tmp/s2.jsonl 2>&1
  local E=$?
  local OK=true
  [ $E -ne 0 ] && OK=false
...
test_tui_interactive() {
  local MHOME=$(mktemp -d) WS=$(mktemp -d) SID="spadak-s3-$$"
  local PROMPT_RE='>|Ask|/[a-z]'
  tmux new-session -d -s "$SID" -x 120 -y 30 "SPADAKCODE_HOME=$MHOME SPADAKCODE_PURE=true spadak $WS; sleep 999"
...
```bash
test_tui_permission() {
  local MHOME=$(mktemp -d) WS=$(mktemp -d) SID="spadak-s4-$$"
  local PROMPT_RE='>|Ask|/[a-z]'
  tmux new-session -d -s "$SID" -x 120 -y 30 "SPADAKCODE_HOME=$MHOME SPADAKCODE_PURE=true spadak $WS; sleep 999"
...
test_tui_keybindings() {
  local MHOME=$(mktemp -d) WS=$(mktemp -d) SID="spadak-s5-$$"
  local PROMPT_RE='>|Ask|/[a-z]'
  tmux new-session -d -s "$SID" -x 120 -y 30 "SPADAKCODE_HOME=$MHOME SPADAKCODE_PURE=true spadak $WS; sleep 999"
...
| Task | Command |
|------|---------|
| Headless run | `SPADAKCODE_HOME=$(mktemp -d) spadak run --format json --dangerously-skip-permissions "prompt"` |
| TUI launch | `tmux new-session -d -s test -x 120 -y 30 "SPADAKCODE_HOME=$(mktemp -d) spadak $WORKSPACE; sleep 999"` |
